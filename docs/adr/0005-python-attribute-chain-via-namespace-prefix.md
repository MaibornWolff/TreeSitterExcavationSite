...
- **Two-stream extractor.** TSE's Python `UsedTypeExtractor` emits one `UsedType` per `(identifier)` *and* one per `(attribute)` node found in the definition body. For `os.path.join` this means up to five `UsedType` entries (three identifiers — `os`, `path`, `join` — plus two attribute nodes — `os.path.join` and `os.path`). This noise mirrors DC; the adapter and resolver silently drop no-ops.
