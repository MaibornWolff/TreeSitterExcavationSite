...
### 7. Release
- **Pre-PR: remove now-redundant `extractPackagePath` stubs from the JS/TS/TSX dependency mappings.** The JS/TS branch (`feat/typescript-javascript-dependency-support`) merged before this one and hardcoded `extractPackagePath = { _, _ -> emptyList() }` in `JavascriptDependencyMapping.kt`, `TypescriptDependencyMapping.kt`, and `TsxDependencyMapping.kt` because the param was still required at the time. Task 1 of this plan made `extractPackagePath` optional with the same empty-list default (see `DependencyMapping.kt`), so those three lines and their `// JS/TS have no package declarations` comments are dead weight — drop them as part of this PR rather than as a follow-up so the released code doesn't carry stale stubs. Sanity check: `git grep "_, _ -> emptyList()" src/main/kotlin/de/maibornwolff/treesitter/excavationsite/languages/javascript src/main/kotlin/de/maibornwolff/treesitter/excavationsite/languages/tsx` returns nothing.
